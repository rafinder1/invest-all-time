...
def get_stock_price(ticker: str, period: str = '1d'):
    ...
    You can find tickers on: https://finance.yahoo.com/
    - period (str): The period for which to retrieve historical data
    (default is '1d'). Options include: '1d', '5d', '1mo', '3mo',
    ...
    try:
        ...
        if data.empty:
            raise HTTPException(
                status_code=404,
                detail="No data for the given ticker or period."
            )
        return data.to_dict()
    except Exception as e:
        raise HTTPException(
            status_code=400,
            detail=f"Data download error: {str(e)}"
        )
